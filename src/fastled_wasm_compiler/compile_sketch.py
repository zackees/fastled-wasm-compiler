...
def compile_sketch(sketch_dir: Path, build_mode: str) -> Exception | None:
    ...
    for i, src_file in enumerate(sources, 1):
        ...
        if cp.returncode != 0:
            ...
            return RuntimeError(f"Error compiling {src_file}: Compilation failed with exit code {cp.returncode}")
    ...
    if cp.returncode != 0:
        ...
        return RuntimeError(f"Error linking {output_js}: Linking failed with exit code {cp.returncode}")
    else:
