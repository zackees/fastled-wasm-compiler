...
from fastled_wasm_compiler.paths import FASTLED_ROOT, FASTLED_SRC
...
class FastLEDDownloader:
    ...
    FASTLED_URL = f"https://github.com/FastLED/FastLED/archive/refs/heads/{FASTLED_VERSION}.zip"
    ...
    def download_and_extract(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with httpx.stream("GET", self.FASTLED_URL, follow_redirects=True, timeout=300) as response:
            ...
            if extracted_dir.exists():
                ...
                raise RuntimeError(f"Expected extracted directory not found: {extracted_dir}")
    ...
    def cleanup_cpp_files(self) -> None:
        ...
        print("Removing *.cpp files (but keeping *.hpp.cpp files and essential WASM platform files)...")
        ...
        for cpp_file in self.fastled_src.rglob("*.cpp"):
            if not cpp_file.name.endswith(".hpp.cpp") and cpp_file.name not in essential_files:
                print(f"Removing: {cpp_file.relative_to(self.fastled_src)}")
                cpp_file.unlink()
            elif cpp_file.name in essential_files:
                print(f"Preserving essential file: {cpp_file.relative_to(self.fastled_src)}")
    ...
    def install(self, force: bool = False) -> None:
        ...
        if not self.is_installed():
            raise RuntimeError(f"FastLED installation failed - FastLED.h not found in {self.fastled_src}")
